...
from genesys.assistants import manuscript_assistant
...
for message in client.beta.threads.messages.list(
    ...
    if role == "assistant" and message.assistant_id == manuscript_assistant.id:
        with st.chat_message(role):
            st.markdown(text_value)
    elif (
            role == "user"
            and isinstance(message.metadata, dict)
            and message.metadata["page"] == "manuscript"
        ):
        with st.chat_message(role):
            st.markdown(text_value)
...
if prompt := st.chat_input("Say something"):
    ...
    with st.chat_message("assistant"):
        run = client.beta.threads.runs.create(
            ...
            assistant_id=manuscript_assistant.id,
        )
        ...
        with st.spinner('Waiting for a response...'):
            while True:
                ...
                if run.status == "requires_action" and run.required_action is not None:
                    tool_outputs = manuscript_assistant.get_tool_outputs(run)
